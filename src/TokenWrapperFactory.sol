...
contract TokenWrapperFactory {
    event TokenWrapperDeployed(
        ERC20 underlyingToken, uint256 unlockTime, TokenWrapper tokenWrapper
    );
    ...
    function deployWrapper(ERC20 underlyingToken, uint256 unlockTime)
        external
        returns (TokenWrapper tokenWrapper)
    {
        ...
        tokenWrapper = TokenWrapper(
            LibClone.cloneDeterministic(
                address(implementation), abi.encode(underlyingToken, unlockTime), salt
            )
        );
